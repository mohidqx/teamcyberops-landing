...
import { useSocialLinks, useSiteContent } from "@/hooks/use-cms";
      ...
      <div className="glass-strong rounded-xl p-2 flex flex-col gap-2">
        {socials.map((s) => (
          <a
            ...
            title={s.platform}
            className="group w-9 h-9 flex items-center justify-center rounded-lg text-muted-foreground hover:text-primary hover:bg-primary/10 transition-all duration-300"
          >
            <svg className="w-4 h-4 fill-current transition-transform group-hover:scale-110" viewBox="0 0 24 24">
              {socialIcons[s.icon] || socialIcons.github}
            </svg>
          </a>
        ))}
      </div>
      ...
      <div className="relative max-w-6xl mx-auto">
        <div className="grid md:grid-cols-3 gap-10 items-start">
          ...
          <div className="flex flex-wrap gap-x-6 gap-y-2 md:justify-center">
            {["Mission", "Arsenal", "Intel", "Crew", "Contact"].map((link) => (
              <button
                key={link}
                onClick={() => document.querySelector(`#${link.toLowerCase()}`)?.scrollIntoView({ behavior: "smooth" })}
                className="font-mono-terminal text-xs text-muted-foreground hover:text-primary transition-colors"
              >
                {link}
              </button>
            ))}
          ...
          <div className="flex gap-2 md:justify-end flex-wrap lg:hidden">
            {socials.map((s) => (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="w-9 h-9 flex items-center justify-center border border-border rounded-lg text-muted-foreground hover:text-primary hover:border-primary/30 transition-all"
              >
                <svg className="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24">{socialIcons[s.icon] || socialIcons.github}</svg>
              </a>
            ))}
          </div>
        </div>
        ...
        <div className="mt-10 pt-6 border-t border-border/30 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="font-mono-terminal text-[10px] text-muted-foreground/60">
            © {new Date().getFullYear()} TeamCyberOps // All rights reserved
          </p>
          <p className="font-mono-terminal text-[10px] text-muted-foreground/40">
            {content?.footer_subtext || "github.com/mohidqx • Offensive Security Research"}
          </p>
        </div>
      </div>
    </footer>
